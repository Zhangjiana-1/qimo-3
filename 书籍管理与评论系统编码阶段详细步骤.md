...
#### 2.2 数据库创建
在VSCode中创建`book_review_system`数据库，并设置正确的字符集和排序规则，有以下两种方法：
...
3. 在弹出的对话框中，输入数据库名称：`book_review_system`
4. 选择字符集：`utf8mb4`
5. 选择排序规则：`utf8mb4_unicode_ci`
6. 点击"确定"按钮，完成数据库创建

完成数据库创建后，可以在VSCode的数据库资源管理器中看到新创建的`book_review_system`数据库。

#### 2.3 数据表创建
在VSCode中按照以下顺序创建数据表（先创建无外键依赖的表，再创建有外键依赖的表）：

1. **创建users表（用户表）**
   - 在VSCode中，确保已连接到MySQL并选择了`book_review_system`数据库
   - 打开一个新的查询编辑器窗口
   - 粘贴以下SQL语句：
```sql
CREATE TABLE `users` (
...
```
   ...
   - 在数据库资源管理器中展开`book_review_system`数据库，检查是否成功创建了users表
...
1. **在VSCode中导入示例数据的通用步骤**
   - 确保已连接到MySQL并选择了`book_review_system`数据库
   - 打开一个新的查询编辑器窗口
...
3. **在VSCode中验证数据导入**
   - 在数据库资源管理器中展开`book_review_system`数据库
   - 点击每个表名，查看表中的数据
